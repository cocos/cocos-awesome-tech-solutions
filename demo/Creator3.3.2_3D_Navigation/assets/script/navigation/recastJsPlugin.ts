...
export class RecastJSPlugin implements INavigationEnginePlugin {
     ...
     * @param initCb
     */
    public constructor(initCb : ()=>void,recastInjection: any = Recast) {
        if (typeof recastInjection === "function") {
            new recastInjection().then(((val : any)=>{
                this.bjsRECAST = val;
                if (!this.isSupported()) {
    ...
    createNavMesh(meshes: Array<cc.MeshRenderer>, parameters: INavMeshParameters): void {
        ...
        let offMeshConfig = null;
        if(parameters.offMeshLinkConfig){
            let config = parameters.offMeshLinkConfig;
            offMeshConfig = this.bjsRECAST.OffMeshLinkConfig.prototype.GetInstance(config.offMeshConVerts,config.offMeshConRad,
                config.offMeshConFlags,config.offMeshConAreas,config.offMeshConDir,config.offMeshConUserID,config.offMeshConCount);
        }
        this.navMesh = new this.bjsRECAST.NavMesh();
        let {vertexTypeArray, startId, facesTypeArray} = RecastJSPlugin.getMeshData(meshes);
    ...
    static getMeshData(meshes : cc.MeshRenderer[]){
        let comps = meshes;
        let vertexPosList = [];
        let facesPositions : (Uint8Array | Uint16Array | Uint32Array)[] = [];
        let vertexLength = 0;
        let facesLength = 0;
        let worldMatrix = new math.Mat4();
        let outPos = cc.v3();
        let group = [];
        let startId = 0;
        for(let i = 0;i < comps.length;++i){
            let model = comps[i].model;
            if(!model){
                continue;
            }
            comps[i].node.getWorldMatrix(worldMatrix);
            const subModels = model.subModels;
            let r = 1 / RecastConfig.RATIO;
            for (let j = 0; j < subModels.length; j++) {
                group.push(startId);
                const subMesh = subModels[j].subMesh;
                let info = subMesh.geometricInfo;
                if(info && info.positions && info.indices){
                    let positions = [];
                    let num = info.positions.length / 3;
                    for(let n = 0;n < num;++n){
                        Vec3.transformMat4(outPos,cc.v3(info.positions[n * 3],info.positions[n * 3 + 1]
                            ,info.positions[n * 3 + 2]),worldMatrix);
        ...
        return {vertexTypeArray, startId, facesTypeArray,group}
    }

    static exportObj(meshes : cc.MeshRenderer[]){
        let output = [];
        let {vertexTypeArray, facesTypeArray,group} = this.getMeshData(meshes);
        for (let h = 0; h < vertexTypeArray.length; h += 3) {
            output.push("v " + vertexTypeArray[h] + " " + vertexTypeArray[h + 1] + " " + vertexTypeArray[h + 2]);
        }
        for (let i = 0; i < facesTypeArray.length; i += 3) {
     ...
     * @param indices
     */
    static createMeshRenderer(scene : cc.Node,positions : number[],indices : number[]){
        let node = new cc.Node();
        scene.addChild(node);
        const m4 = new math.Mat4();
...
export class RecastJSCrowd implements ICrowd {
     ...
     * @returns the crowd you can add agents to
     */
    public constructor(plugin: RecastJSPlugin, maxAgents: number, maxAgentRadius: number, scene: cc.Scene) {
     ...
     * @param recastPos
     */
    static getWorldPosition(recastPos : cc.Vec3){
