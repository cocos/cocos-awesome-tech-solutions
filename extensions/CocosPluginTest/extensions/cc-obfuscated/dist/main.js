 ...
 */
var obfuscate = function (filePath, options) {
    ...
    if (params.state == "success" || params.progress == 1) {
        ...
        if (Fs.existsSync(getWebMobilePath_subpackages_0) || Fs.existsSync(getWebMobilePath_subpackages_1)) {
            try {
                ...
                console.error("❌ 构建结束 error=>", error);
            }
            ;
        }
        else {
        ...
        if (isn_open_buildOb) {
            ...
            console.log("⭐ 正在开始混淆 [" + params.options.platform + "] 里面的代码");
            // 合并混淆
            Ast_Codeing_Do.ast_mix_jsMAIN(temp_array[0], isn_open_buildOb, params, obfuscate, getWebMobilePath_subpackages_0, getWebMobilePath_subpackages_1, getWebMobilePath_main, temp_array, tmp_item, getCongif);
        }
        else {
